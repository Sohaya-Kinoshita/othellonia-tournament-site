...
// .wrangler/tmp/pages-Ts2T2f/functionsWorker-0.030378651482458108.mjs
...
async function readCurrentUserFromSession(request, db) {
  ...
  if (userType === "admin") {
    const adminRow = await db.prepare("SELECT userId FROM admins WHERE userId = ?").bind(sessionRow.userId).first();
...
__name2(readCurrentUserFromSession, "readCurrentUserFromSession");
...
__name2(createPasswordHash, "createPasswordHash");
...
__name2(pbkdf2HashBase64, "pbkdf2HashBase64");
...
async function onRequestGet(context) {
  const request = context.request;
  const db = context.env.DB;
  const currentUser = await readCurrentUserFromSession2(request, db);
  if (!currentUser || currentUser.role !== "admin") {
...
async function readCurrentUserFromSession2(request, db) {
  const sessionId = readCookie2(request, "sessionId");
  if (!sessionId) return null;
  const sessionRow = await db.prepare("SELECT userId, userType, expiresAt FROM sessions WHERE sessionId = ?").bind(sessionId).first();
  if (!sessionRow) return null;
  if (new Date(sessionRow.expiresAt).getTime() < Date.now()) return null;
  const userType = sessionRow.userType;
  if (userType === "admin") {
...
__name(readCurrentUserFromSession2, "readCurrentUserFromSession2");
__name2(readCurrentUserFromSession2, "readCurrentUserFromSession");
...
async function onRequestPost2(context) {
  ...
  try {
    ...
    return jsonResponse3({ ok: false, message: "JSON\u304C\u4E0D\u6B63\u3067\u3059\uFF0E" }, 400);
  }
  ...
  const adminRow = await db.prepare("SELECT userId, passwordHash FROM admins WHERE userId = ?").bind(userId).first();
  ...
  ).bind(sessionId, userRow.userId, userType, expiresAt).run();
...
__name(pbkdf2HashBase642, "pbkdf2HashBase642");
__name2(pbkdf2HashBase642, "pbkdf2HashBase64");
...
async function onRequestGet2(context) {
  const db = context.env.DB;
  const request = context.request;
  const sessionId = readCookie4(request, "sessionId");
  if (!sessionId) {
    return jsonResponse4({ ok: true, isLoggedIn: false }, 200);
  }
  const sessionRow = await db.prepare(
  ...
  let userRow;
  if (userType === "admin") {
    userRow = await db.prepare("SELECT userId FROM admins WHERE userId = ?").bind(sessionRow.userId).first();
  } else if (userType === "participant") {
...
function jsonResponse4(body, status, extraHeaders) {
  return new Response(JSON.stringify(body), {
    status,
    headers: {
      "Content-Type": "application/json; charset=utf-8",
...
function readCookie4(request, cookieName) {
  const cookieHeader = request.headers.get("Cookie") ?? "";
  const cookies = cookieHeader.split(";").map((c) => c.trim());
  for (const cookie of cookies) {
    if (!cookie.startsWith(cookieName + "=")) continue;
...
__name(clearSessionCookie2, "clearSessionCookie2");
__name2(clearSessionCookie2, "clearSessionCookie");
var routes = [
  {
...
// .wrangler/tmp/bundle-tT2TKq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-tT2TKq/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.030378651482458108.js.map
